...
def main():
    ...
    for f in sorted(glob(args.glob)):
        ...
        with opener(f, 'rt', errors='replace') as fh:
            for line in fh:
                ...
                # `?src=launch` — a one-off broadcast to every account with a
                # build, the FC 27 launch-day email first (app repo #212).
                if 'src=launch' in path_q:
                    days[day]['launch_clicks'] += 1
                if blog_ref:
    ...
    hdr = (f'{"day":<12}{"blog views":>11}{"blog vis.":>10}{"→app":>6}'
           f'{"app views":>10}{"app vis.":>9}{"hydr.":>7}{"card":>6}{"grid":>6}{"guide":>7}{"dgst":>6}{"lnch":>6}'
    ...
    for d in all_days:
        ...
        print(f'{d:<12}{c["blog_views"]:>11}{len(blog_visitors[d]):>10}'
              ...
              f'{c["card_clicks"]:>6}{c["grid_clicks"]:>6}{c["guide_clicks"]:>7}{c["digest_clicks"]:>6}{c["launch_clicks"]:>6}{c["registers"]:>5}'
    ...
    print(f'{"total":<12}{tot["blog_views"]:>11}{tot["blog_vis"]:>10}'
          ...
          f'{tot["hydrations"]:>7}{tot["card_clicks"]:>6}{tot["grid_clicks"]:>6}{tot["guide_clicks"]:>7}{tot["digest_clicks"]:>6}{tot["launch_clicks"]:>6}'
