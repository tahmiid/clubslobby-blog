// Publishes the four articles to the LOCAL staging Ghost. Ghost converts the
// HTML to Lexical itself, which is why we author here and export rather than
// hand-writing Lexical for the import file.
...
for (const p of POSTS) {
  ...
  const body = {
    title: p.title, slug: p.slug, html, status: p.status,
  ...
  let res;
  if (found.ok) {
    const existing = (await found.json()).posts[0];
    res = await call(`/posts/${existing.id}/?source=html`, {
      method: 'PUT', body: JSON.stringify({ posts: [{ ...body, updated_at: existing.updated_at }] }),
    });
  } else {
